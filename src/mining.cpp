...
// ── Nonce batch sizes ──────────────────────────────────────────────────────
// Larger batches = fewer mutex lock/unlock cycles per second = more hashing
// SW: 4x original (4096→16384). HW: 2x original (16384→32768)
#define NONCE_PER_JOB_SW  16384
#define NONCE_PER_JOB_HW  32*1024

// ── Queue depth ────────────────────────────────────────────────────────────
// 8 jobs queued at all times so both workers never sit idle waiting
#define JOB_QUEUE_DEPTH   8

//#define I2C_SLAVE

//#define SHA256_VALIDATE
// RANDOM_NONCE: Each job starts at a random nonce instead of sequential.
// This prevents both workers hashing the SAME nonce range when restarting.
// This is the "duplicate nonce" bug — fixed here.
#define RANDOM_NONCE
#define RANDOM_NONCE_MASK 0xFFFF0000
...
// TEST: https://bitcoin.stackexchange.com/questions/22929/full-example-data-for-scrypt-stratum-client
  ...
  // Separate nonce pools for SW and HW workers.
  // This prevents SW and HW from ever hashing the same nonce range.
  uint32_t nonce_pool_sw = 0;
  uint32_t nonce_pool_hw = 0x80000000; // HW starts from upper half of nonce space
  ...
  while(true) {
          ...
          case MINING_NOTIFY:         if(parse_mining_notify(line, mJob))
                                      {
                                          ...
                                          nonce_pool_hw = (RandomGet() & RANDOM_NONCE_MASK) | 0x80000000;
                                          #else
                                            #ifdef I2C_SLAVE
                                            if (!i2c_slave_vector.empty())
                                              nonce_pool_sw = 0x10000000;
                                            else
                                            #endif
                                              nonce_pool_sw = 0xDA54E700;
                                              nonce_pool_hw = 0x5A54E700; // different start for HW
                                          #endif

                                          {
                                            std::lock_guard<std::mutex> lock(s_job_mutex);
                                            // Fill JOB_QUEUE_DEPTH jobs for each worker type
                                            // Each job gets a non-overlapping nonce range
                                            for (int i = 0; i < JOB_QUEUE_DEPTH; ++i)
                                            {
                                              JobPush(s_job_request_list_sw, job_pool, nonce_pool_sw, NONCE_PER_JOB_SW, currentPoolDifficulty, mMiner.bytearray_blockheader, diget_mid, bake);
                                              #ifdef RANDOM_NONCE
                                              nonce_pool_sw = RandomGet() & RANDOM_NONCE_MASK;
                                              #else
                                              nonce_pool_sw += NONCE_PER_JOB_SW;
                                              #endif

                                              #ifdef HARDWARE_SHA265
                                                #if defined(CONFIG_IDF_TARGET_ESP32)
                                                  JobPush(s_job_request_list_hw, job_pool, nonce_pool_hw, NONCE_PER_JOB_HW, currentPoolDifficulty, sha_buffer_swap, hw_midstate, bake);
                                                #else
                                                  JobPush(s_job_request_list_hw, job_pool, nonce_pool_hw, NONCE_PER_JOB_HW, currentPoolDifficulty, mMiner.bytearray_blockheader, hw_midstate, bake);
                                                #endif
                                                #ifdef RANDOM_NONCE
                                                nonce_pool_hw = (RandomGet() & RANDOM_NONCE_MASK) | 0x80000000;
                                                #else
                                                nonce_pool_hw += NONCE_PER_JOB_HW;
                                                #endif
                                              #endif
                                            }
                                          }
    ...
    if (job_pool != 0xFFFFFFFF)
    {
      ...
      // Refill SW queue - only lock while pushing, work is pre-calculated
      while (s_job_request_list_sw.size() < JOB_QUEUE_DEPTH)
      {
        JobPush(s_job_request_list_sw, job_pool, nonce_pool_sw, NONCE_PER_JOB_SW, currentPoolDifficulty, mMiner.bytearray_blockheader, diget_mid, bake);
        #ifdef RANDOM_NONCE
        nonce_pool_sw = RandomGet() & RANDOM_NONCE_MASK;
      ...
      #ifdef HARDWARE_SHA265
      // Refill HW queue — separate nonce space, no overlap with SW
      while (s_job_request_list_hw.size() < JOB_QUEUE_DEPTH)
      {
        #if defined(CONFIG_IDF_TARGET_ESP32)
          JobPush(s_job_request_list_hw, job_pool, nonce_pool_hw, NONCE_PER_JOB_HW, currentPoolDifficulty, sha_buffer_swap, hw_midstate, bake);
        #else
...
void minerWorkerSw(void * task_id)
{
  ...
  Serial.printf("[MINER] SW worker %d started on core %d\n", miner_id, xPortGetCoreID());
  ...
  while (1)
  {
    // ── LOCK SECTION: only queue access inside lock ──────────────────────
    {
      std::lock_guard<std::mutex> lock(s_job_mutex);

      // Push previous result while we have the lock
      if (result)
      {
    ...
    // ── END LOCK: all hashing work is outside the lock ───────────────────

    if (job)
    {
      // Pre-copy job data to local vars — avoids repeated pointer chasing in loop
      const uint32_t nonce_start  = job->nonce_start;
      const uint32_t nonce_count  = job->nonce_count;
      const uint8_t  job_in_work  = job->id & 0xFF;
      ...
      // ── Main hashing loop ─────────────────────────────────────────────
      // All SHA256 work happens here — completely outside any lock
      for (uint32_t n = 0; n < nonce_count; ++n)
      {
        // Write nonce directly into the buffer
        ((uint32_t*)(job->sha_buffer + 64 + 12))[0] = nonce_start + n;
        ...
        if (nerd_sha256d_baked(job->midstate, job->sha_buffer + 64, job->bake, hash))
        {
          ...
          if (diff_hash > result->difficulty)
          {
            ...
            result->nonce      = nonce_start + n;
            memcpy(result->hash, hash, 32);
          }
        }

        // Check for job cancellation every 256 nonces
        // (checking every nonce would hurt performance)
      ...
      // No job available — yield immediately instead of sleeping 2ms
      // taskYIELD lets the other task on this core run and come back fast
      taskYIELD();
    }

    // WDT reset every 4 job loops (was 8)
    // Needed because NONCE_PER_JOB_SW is now larger
    wdt_counter++;
    if (wdt_counter >= 4)
    {
      wdt_counter = 0;
