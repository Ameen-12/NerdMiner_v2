...
strncpy(mWorker.wPass, Settings.PoolPassword, sizeof(mWorker.wPass) - 1);
mWorker.wPass[sizeof(mWorker.wPass) - 1] = '\0';
    ...
    if (job_pool != 0xFFFFFFFF)
    {
...
#if 1
      while (s_job_request_list_sw.size() < 8)
      {
        JobPush( s_job_request_list_sw, job_pool, nonce_pool, NONCE_PER_JOB_SW, currentPoolDifficulty, mMiner.bytearray_blockheader, diget_mid, bake);
        #ifdef RANDOM_NONCE
        nonce_pool = RandomGet() & RANDOM_NONCE_MASK;
      ...
      #ifdef HARDWARE_SHA265
      while (s_job_request_list_hw.size() < 8)
      {
        #if defined(CONFIG_IDF_TARGET_ESP32)
          JobPush( s_job_request_list_hw, job_pool, nonce_pool, NONCE_PER_JOB_HW, currentPoolDifficulty, sha_buffer_swap, hw_midstate, bake);
        #else
...
void minerWorkerSw(void * task_id)
{
  ...
  while (1)
  {
    ...
    if (job)
    {
      ...
      uint32_t end = job->nonce_start + job->nonce_count;
      for (uint32_t n = job->nonce_start; n < end; ++n)
      {
...
void minerWorkerHw(void * task_id)
{
  ...
  while (1)
  {
    ...
    if (job)
    {
      ...
      result->difficulty = 0.0;
      uint8_t job_in_work = job->id & 0xFF;
      memcpy(sha_buffer, job->sha_buffer, 128);
...
void runMonitor(void *name)
{
  ...
  while (1)
  {
    ...
    { 
      ...
      if (elapsedKHs == 0)
      {
        // hashrate is zero - no action needed
      }
