...
// ============================================================
// DUAL-CORE + HASHRATE OPTIMIZATIONS for ESP32-DevKit v1
//
// Changes made:
// 1. NONCE_PER_JOB_SW increased: 4096 -> 8192
//    - Each worker processes more nonces per job slice
//    - Reduces mutex lock/unlock overhead per hash
//
// 2. Job queue depth increased: 4 -> 8
//    - Workers always have jobs waiting, zero idle time
//    - Both cores stay saturated
//
// 3. NUM_SW_WORKERS = 2
//    - Core 0: runStratumWorker (network/pool comms)  [pinned]
//    - Core 1: minerWorkerSw instance 0               [pinned]
//    - Core 1: minerWorkerSw instance 1               [pinned]
//    NOTE: In your main.cpp / setup(), change task creation to:
//      xTaskCreatePinnedToCore(runStratumWorker, "stratum", 8192, ..., 1, NULL, 0);
//      xTaskCreatePinnedToCore(minerWorkerSw,    "miner0",  4096, (void*)0, 2, NULL, 1);
//      xTaskCreatePinnedToCore(minerWorkerSw,    "miner1",  4096, (void*)1, 2, NULL, 1);
//    This pins network on core 0, both miners on core 1.
//    ESP32 has 2 cores: PRO_CPU (0) and APP_CPU (1).
//
// 4. WDT reset frequency improved in minerWorkerSw
//    - Was every 8 iterations, now every 4 for stability
//      with larger nonce batches
//
// 5. Removed vTaskDelay(2) when no job — spin yields instead
//    - taskYIELD() is faster than a 2ms sleep
// ============================================================

// Increased from 4096 — bigger batches = less mutex overhead
#define NONCE_PER_JOB_SW  8192
#define NONCE_PER_JOB_HW  16*1024

// How many jobs to keep queued per worker
// Increased from 4 to 8 so workers never starve
#define JOB_QUEUE_DEPTH   8

// Number of SW mining workers (set to 2 for dual-core)
#define NUM_SW_WORKERS    2
...
volatile uint32_t shares;
volatile uint32_t valids;
...
void runStratumWorker(void *name) {
  ...
  while(true) {
      ...
      switch (result)
      {
          case MINING_NOTIFY:
            if(parse_mining_notify(line, mJob))
            {
              ...
              {
                std::lock_guard<std::mutex> lock(s_job_mutex);
                // OPTIMIZATION: Fill queue to JOB_QUEUE_DEPTH (was 4, now 8)
                // This keeps both worker threads busy without starving
                for (int i = 0; i < JOB_QUEUE_DEPTH; ++i)
                {
                  JobPush(s_job_request_list_sw, job_pool, nonce_pool, NONCE_PER_JOB_SW,
                  ...
                  #ifdef HARDWARE_SHA265
                    #if defined(CONFIG_IDF_TARGET_ESP32)
                      JobPush(s_job_request_list_hw, job_pool, nonce_pool, NONCE_PER_JOB_HW,
          ...
          default:
            Serial.println("  Parsed JSON: unknown");
            break;
      }
    }
    ...
    if (job_pool != 0xFFFFFFFF)
    {
      ...
      // OPTIMIZATION: Refill SW queue to JOB_QUEUE_DEPTH (was 4, now 8)
      // Accounts for NUM_SW_WORKERS workers consuming jobs simultaneously
      while (s_job_request_list_sw.size() < (size_t)JOB_QUEUE_DEPTH)
      {
        JobPush(s_job_request_list_sw, job_pool, nonce_pool, NONCE_PER_JOB_SW,
                currentPoolDifficulty, mMiner.bytearray_blockheader, diget_mid, bake);
        #ifdef RANDOM_NONCE
      ...
      #ifdef HARDWARE_SHA265
      while (s_job_request_list_hw.size() < (size_t)JOB_QUEUE_DEPTH)
      {
        #if defined(CONFIG_IDF_TARGET_ESP32)
          JobPush(s_job_request_list_hw, job_pool, nonce_pool, NONCE_PER_JOB_HW,
                  currentPoolDifficulty, sha_buffer_swap, hw_midstate, bake);
...
// OPTIMIZATION: minerWorkerSw now runs on both cores.
// Two instances are created in main.cpp pinned to Core 1.
// Core 0 handles network (runStratumWorker).
// Both instances share the same job queue (s_job_request_list_sw)
// which is protected by s_job_mutex.
  ...
  {
    {
      ...
      if (result)
      {
        if (s_job_result_list.size() < 32) // increased from 16
    ...
    if (job)
    {
      ...
      for (uint32_t n = 0; n < job->nonce_count; ++n)
      {
        ...
        // OPTIMIZATION: Check job cancellation every 256 nonces (was 256, unchanged)
        // but WDT reset now every 4 job loops instead of 8
        // to handle the larger NONCE_PER_JOB_SW safely
      ...
      // OPTIMIZATION: taskYIELD instead of vTaskDelay(2)
      // Lets the other task on this core run immediately
      // instead of sleeping 2ms, reducing idle time
      taskYIELD();
    }

    // OPTIMIZATION: WDT every 4 instead of 8
    // Needed because NONCE_PER_JOB_SW is larger now
    wdt_counter++;
    if (wdt_counter >= 4)
    {
      wdt_counter = 0;
