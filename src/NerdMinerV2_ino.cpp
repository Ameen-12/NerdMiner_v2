...
void setup()
{
  ...
  // ── MINER TASKS ────────────────────────────────────────────────────────
  // Both miners pinned to Core 0 (PRO_CPU)
  // Core 1 (APP_CPU) = Monitor + Stratum = network only
  // This gives each core a dedicated role with zero contention
  TaskHandle_t minerTask1, minerTask2 = NULL;
  #ifdef HARDWARE_SHA265
    #if defined(CONFIG_IDF_TARGET_ESP32)
    // Increased stack 3584→4096 for stability with larger nonce batches
  ...
  #else
    #if defined(CONFIG_IDF_TARGET_ESP32)
    // Increased stack 5000→6000 for stability with larger nonce batches
    xTaskCreatePinnedToCore(minerWorkerSw, "MinerSw-0", 6000, (void*)0, 1, &minerTask1, 0);
    #else
    xTaskCreatePinnedToCore(minerWorkerSw, "MinerSw-0", 6000, (void*)0, 1, &minerTask1, 0);
    #endif
  #endif
  ...
  // Second SW worker also pinned to Core 0
  // Two miners on one core, network on the other
  #if defined(CONFIG_IDF_TARGET_ESP32)
